...
7. 並列で割り当てた Issue は、各 feature ブランチの commit、push、PR 作成までを完了条件として追跡する。
8. 最終報告前に push 済み feature ブランチと open PR の対応を確認し、PR 作成漏れがあれば作成する。
9. Member の PR を一次レビューし、受け入れ条件との一致と明らかな問題を確認する。
10. planモードを用いて、実装するだけの所まで計画を立てる。（人に問い合わせが必要な場合は、このフェイズで問い合わせを行う。）
11. レビュー指摘を読み、元 Member または別 Member に再割り当てする。
