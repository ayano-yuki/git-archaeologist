...
6. 人間確認後、`gh issue create --parent 58` コマンド案を提示または実行する。
7. 作成済み Issue が親 #58 の sub-issue になっていることを確認し、抜けがあれば `gh issue edit <issue-number> --parent 58` で修復する。
8. 並列で割り当てた Issue は、各 feature ブランチの commit、push、PR 作成までを完了条件として追跡する。
9. 最終報告前に push 済み feature ブランチと open PR の対応を確認し、PR 作成漏れがあれば作成する。
10. Member の PR を一次レビューし、受け入れ条件との一致と明らかな問題を確認する。
11. planモードを用いて、実装するだけの所まで計画を立てる。（人に問い合わせが必要な場合は、このフェイズで問い合わせを行う。）
12. レビュー指摘を読み、元 Member または別 Member に再割り当てする。

## レビューチェックリスト

- Issue の受け入れ条件を満たしているか。
- push 済み feature ブランチに対応する PR が作成されているか。
- 子 Issue が親 #58 の GitHub sub-issue として紐付いているか。
