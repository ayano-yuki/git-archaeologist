...
各 gold case は `bundle_id`, `question`, `answer`, `facts`, `timeline`, `inference`, `uncertainty`, `citations`, `review_status` を持ち、SFT に使うものは `review_status: approved` にします。approved caseには `reviewer_id`, `reviewed_at`, `review_revision`, `bundle_hash`, `evidence_hash` も必要です。
